...
def marketplace_module():
    ...
    class REJKTMarketplace(sp.Contract):
        ...
        def collect(self, params):
            ...
            paid = sp.local("paid", sp.amount)

            # 1. Platform fee
            if self.data.fee_percent > sp.nat(0):
                fee = sp.split_tokens(paid.value, self.data.fee_percent, sp.nat(1000))
                if fee > sp.mutez(0):
                    sp.send(self.data.fee_recipient, fee)
                    paid.value = paid.value - fee
            ...
            for recipient, share in royalty_map.items():
                ...
                if royalty_pmt > sp.mutez(0):
                    if royalty_pmt <= paid.value:
                        sp.send(recipient, royalty_pmt)
                        paid.value = paid.value - royalty_pmt

            # 3. Remainder to seller
            if paid.value > sp.mutez(0):
                sp.send(swap.seller, paid.value)
