...
- The marketplace adds itself as FA2 operator on swap, removes on cancel/sold-out.
"""
...
def marketplace_module():
    ...
    class StubFA2(sp.Contract):
        """Minimal FA2 stub for marketplace tests."""
        ...
        def set_balance(self, params):
            """Test helper: seed a ledger balance directly."""
            self.data.ledger[(params.owner, params.token_id)] = params.amount
        ...
        def transfer(self, batch):
            ...
            for xfer in batch:
                for tx in xfer.txs:
                    from_key = (xfer.from_, tx.token_id)
    ...
    class REJKTMarketplace(sp.Contract):
        ...
        @sp.private()
        def _update_operators(self, params):
            """Send an update_operators call to an FA2 contract."""
            sp.cast(params, sp.record(
                fa2     = sp.address,
                actions = fa2_update_operators_params,
            ))
            fa2_ops = sp.contract(
            ...
            sp.transfer(params.actions, sp.mutez(0), fa2_ops)
        ...
        def swap(self, params):
            ...
            Grants marketplace operator rights so it can transfer at collect time.

            params:
              - token_id : sp.nat
              - amount   : sp.nat    (min 1)
              - price    : sp.mutez  (must be > 0)
            """
            assert params.amount >= sp.nat(1),   "MIN_ONE_EDITION"
            assert params.price  >  sp.mutez(0), "PRICE_MUST_BE_POSITIVE"

            self._update_operators(sp.record(
                fa2     = self.data.fa2_contract,
                actions = [sp.variant.add_operator(sp.record(
                    owner    = sp.sender,
        ...
        def collect(self, params):
            ...
            Payment distributed as:
              1. Platform fee  → fee_recipient  (if fee_percent > 0)
              2. Royalties     → recipients from FA2 get_royalties view
              3. Remainder     → seller
            ...
            # Payment distribution — all shares computed from gross sp.amount.
            # sp.split_tokens rounds down so rounding dust goes to seller.
            ...
            # 2. Royalties — each recipient gets their share of the gross
            royalty_map = sp.View(swap.fa2, "get_royalties")(swap.token_id)
            sp.cast(royalty_map, sp.map[sp.address, sp.nat])

            royalty_total = sp.mutez(0)
            for recipient, share in royalty_map.items():
                royalty_pmt = sp.split_tokens(sp.amount, share, sp.nat(1000))
                if royalty_pmt > sp.mutez(0):
                    sp.send(recipient, royalty_pmt)
                royalty_total = royalty_total + royalty_pmt

            # 3. Seller gets remainder after fee and royalties
            seller_pmt = sp.amount - fee - royalty_total
            if seller_pmt > sp.mutez(0):
                sp.send(swap.seller, seller_pmt)
            ...
            if new_amount == sp.nat(0):
                self._update_operators(sp.record(
                    fa2     = swap.fa2,
                    actions = [sp.variant.remove_operator(sp.record(
                        owner    = swap.seller,
                        operator = sp.self_address,
                        token_id = swap.token_id,
                    ))],
        ...
        # ── set_fa2_contract (admin) ───────────────────────────────────────

        @sp.entrypoint
        def set_fa2_contract(self, new_fa2):
            """Admin: point marketplace at a new FA2 contract."""
...
def test():
    ...
    scenario.verify(
        fa2.data.ledger[(alice.address, sp.nat(0))] == sp.nat(10)
    )
    ...
    scenario.h2("4. Bob buys 2 editions (4 tez total)")
    market.collect(
        sp.record(swap_id=sp.nat(0), quantity=sp.nat(2)),
        _sender=bob,
        _amount=sp.tez(4)
    )
    scenario.verify(fa2.data.ledger[(bob.address, sp.nat(0))] == sp.nat(2))
    ...
    market.collect(
        ...
        _sender=bob, _amount=sp.tez(1), _valid=False
    )
    ...
    market.collect(
        sp.record(swap_id=sp.nat(0), quantity=sp.nat(1)),
        _sender=alice, _amount=sp.tez(2), _valid=False
    )
    ...
    market.collect(
        ...
        _sender=bob, _amount=sp.tez(20), _valid=False
    )
    ...
    # Alice has 10 - 2 - 3 = 5 editions left; list 2 at 10 tez
    market_fee.swap(
        sp.record(token_id=sp.nat(0), amount=sp.nat(2), price=sp.tez(10)),
        _sender=alice
    )
    # Bob collects 1 (10 tez):
    #   fee     = 10 * 25/1000  = 0.25 tez → admin
    #   royalty = 10 * 100/1000 = 1.0  tez → carol
    #   seller  = remainder     = 8.75 tez → alice
    market_fee.collect(
        sp.record(swap_id=sp.nat(0), quantity=sp.nat(1)),
        _sender=bob, _amount=sp.tez(10)
    )
    # Bob: 2 (test4) + 3 (test10) + 1 (test11) = 6
    scenario.verify(fa2.data.ledger[(bob.address, sp.nat(0))] == sp.nat(6))
